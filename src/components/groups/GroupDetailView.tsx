...
export function GroupDetailView({ groupId }: GroupDetailViewProps) {
  ...
  useEffect(() => {
    if (!isLoading && group && !group.invite_link) {
      sync();
    }
  ...
  if (isError || !group) {
    return (
      <div className="flex flex-col items-center justify-center py-20 bg-red-500/5 rounded-[40px] border border-red-500/10">
        <Info className="w-12 h-12 text-red-500/40 mb-4" />
        <h2 className="text-xl font-bold text-white/90">Grupo não encontrado</h2>
        ...
        <Link href="/grupos" className="mt-8">
          <KineticButton className="bg-white/5 shadow-none border border-white/5 hover:bg-white/10 text-white/60">Voltar para listagem</KineticButton>
        </Link>
      </div>
    );
  }

  const admins = participants.filter(p => p.role === 'admin' || p.role === 'creator' || p.role === 'superadmin');
  const members = participants.filter(p => !['admin', 'creator', 'superadmin'].includes(p.role));
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="flex items-center gap-6">
          <div className="relative group">
            <Avatar className="w-24 h-24 rounded-[32px] shadow-skeuo-elevated border-2 border-white/5 ring-4 ring-deep-void">
            ...
            <div className="absolute -bottom-1 -right-1 w-8 h-8 rounded-xl bg-deep-void border border-white/10 flex items-center justify-center shadow-glow-orange">
        ...
        <div className="flex items-center gap-3">
          <KineticButton 
            ...
            className="bg-white/5 border border-white/5 h-12 px-6 shadow-none hover:bg-white/10"
          >
      ...
      {(isError as any)?.response?.data?.error === 'session_disconnected' && (
        <TactileCard className="p-6 bg-red-500/5 border border-red-500/20 shadow-glow-red animate-in fade-in slide-in-from-top-4 duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                participants.map((p) => (
                  <div key={p.remote_id} className="p-4 flex items-center justify-between group hover:bg-white/[0.01] transition-colors">
                    <div className="flex items-center gap-4">
                      <Avatar className="w-10 h-10 rounded-xl shadow-skeuo-flat border border-white/5">
                    ...
                    <div className="flex items-center gap-4">
                      {p.role === 'creator' ? (
                        <div className="flex items-center gap-1.5 px-2 py-0.5 rounded bg-kinetic-orange/10 border border-kinetic-orange/30">
        ...
        <div className="space-y-8">
          <TactileCard className="p-6 space-y-6">
            ...
            <div className="space-y-4">
              <div className="space-y-1">
                ...
                <div className="p-3 rounded-xl bg-deep-void/50 border border-white/5 text-xs text-white/60 leading-relaxed italic">
                  {group.description || 'Nenhuma descrição fornecida pela malha.'}
                </div>
              </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-white/70 font-medium flex items-center gap-1.5">
                      ...
                      {group.remote_created_at ? new Date(group.remote_created_at).toLocaleDateString() : 'N/A'}
                    </p>
                  </div>
                  <div className="space-y-1">
          ...
          <TactileCard className="p-6 space-y-6 bg-emerald-500/[0.02] border-emerald-500/5">
            <h3 className="text-sm font-black uppercase tracking-widest text-white/30 flex items-center gap-2">
            ...
            <div className="space-y-3">
              {[
                { label: 'Somente Admins mandam', active: group.permissions?.announcement },
                { label: 'Restringir Edição', active: group.permissions?.restrict },
                { label: 'Entrada aprovada', active: group.permissions?.joinApprovalMode }
              ].map((perm, idx) => (
                <div key={idx} className="flex items-center justify-between p-3 rounded-2xl bg-deep-void/40 border border-white/5">
